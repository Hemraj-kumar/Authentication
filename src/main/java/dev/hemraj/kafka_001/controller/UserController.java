...
public class UserController {
    ...
    public ResponseEntity<ApiResponse> login(@RequestBody LoginDto loginDto){
        ...
        try{
            apiResponse = authService.loginUser(loginDto);
            if(!apiResponse.getErrorBOList().isEmpty()){
                apiResponse.setErrorBOList(apiResponse.getErrorBOList());
                apiResponse.setCode(500);
                apiResponse.setSuccess(Boolean.FALSE);
            }
        }catch (Exception err){
            log.error("Error in login user request body : {}",loginDto,err);
        }
